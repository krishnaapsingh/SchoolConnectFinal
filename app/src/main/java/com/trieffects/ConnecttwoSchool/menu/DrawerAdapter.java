...
public class DrawerAdapter extends RecyclerView.Adapter<DrawerAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
            items.get(position).bindViewHolder(holder,position);
    ...
    public void setSelected(int position) {
        ...
        if (listener != null) {
            if (position >= 2 && PrefrencesUtils.getUserMode().equalsIgnoreCase("1")) {
                position = position + 2;
            }
            if (position >= 3 && PrefrencesUtils.getUserMode().equalsIgnoreCase("2")){
                position = position + 2;
            }
